    ...
    fn resolve(
        ...
        match account.deposits.get_mut(&transaction_id) {
            Some(deposit) => match deposit.status {
                ...
                _ => return Err(ResolverError::NonDisputedTransaction(transaction_id)),
            },
            None => Err(ResolverError::NoTransactionFound(transaction_id)),
        }
    }
}
...
mod tests {
    ...
    #[case(active(7,    0, vec![(0, accepted_dep(3))]),  0, Err(NonDisputedTransaction(0)), active( 7,    0, vec![(0, accepted_dep(3))]) )]
    #[case(active(7,    0, vec![(0, chrgd_bck_dep(3))]), 0, Err(NonDisputedTransaction(0)), active( 7,    0, vec![(0, chrgd_bck_dep(3))]))]
    #[case(active(7,    0, vec![(0, chrgd_bck_dep(3))]), 1, Err(NoTransactionFound(1)),     active( 7,    0, vec![(0, chrgd_bck_dep(3))]))]
    ...
    fn returns_bad_transaction_when_no_matching_transaction() {
        ...
        assert_eq!(
            ...
            Err(ResolverError::NoTransactionFound(1))
        );
    }
